...
import ModalBase from 'ui/ModelBase'
import NotificationsPanel from '@components/NotificationsPanel'

// hooks
import { SidebarContextType, useSidebar } from '@contexts/sidebarContext'
...
//constants
import { LOCALES } from '@constants/option'
...
const LocaleMenu = ({ active, setActive }: LocaleMenuProps) => {
  return (
...
const AppBar = () => {
  ...
  const [locale, setLocale] = useState<string>('en-EN')
  const { width } = useWindowSize()
  const { theme, toggleTheme } = useTheme() as ThemeContextType
  const { setOpen } = useSidebar() as SidebarContextType
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-5 md:ml-5 xl:gap-[26px]'>
            ...
            <div className='relative'>
              <button
                className='h-8 w-8 rounded-full bg-accent text-widget text-sm flex items-center justify-center
              relative xl:w-11 xl:h-11 xl:text-lg'
                onClick={() => navigate('general-settings')}
                aria-label='Account menu'
              >
                <i className='icon-user-solid' />
              </button>
              <span className='badge-oncline' />
            </div>
          </div>
        </div>
      </Headroom>
